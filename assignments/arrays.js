...
const res = findObj(inventory, 33);
console.log(`Car 33 is a ${res["car_year"]} ${res["car_model"]}`);

// ==== Challenge 2 ====
// The dealer needs the information on the last car in their inventory.  What is the make and model of the last car in the inventory?  Log the make and model into the console.
let lastCar = inventory[inventory.length - 1];
console.log(`${lastCar.car_year} and ${lastCar.car_model}`);
...
function sortCars(arr) {
  ...
  for (const car of arr) {
    res.push(car.car_model);
  }
  return res;
}
let carModels = sortCars(inventory);
...
console.log(sortByName(carModels));

// ==== Challenge 4 ====
// The accounting team needs all the years from every car on the lot. Create a new array from the dealer data containing only the car years and log the result in the console.
function collectYears(arr) {
  const res = [];
  for (const car of arr) {
    res.push(car.car_year);
  }
  return res;
}
let carYears = collectYears(inventory);
console.log(carYears);

// ==== Challenge 5 ====
// The car lot manager needs to find out how many cars are older than the year 2000. Using the carYears array you just created, find out how many cars were made before the year 2000 by populating the array oldCars and logging it's length.
function filterYears(arr, reference) {
  const res = [];
...
let oldCars = filterYears(carYears, 2000);
console.log(oldCars.length);

// ==== Challenge 6 ====
// A buyer is interested in seeing only BMW and Audi cars within the inventory.  Return an array that only contains BMW and Audi cars.  Once you have populated the BMWAndAudi array, use JSON.stringify() to show the results of the array in the console.
function germans(arr) {
  const res = [];
  for (const car of arr) {
...
let BMWAndAudi = germans(inventory);
